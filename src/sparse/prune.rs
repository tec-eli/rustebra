...
use super::{CscError, CscMatrix, CsrError, CsrMatrix};
...
/// pruned non-zero count exceeds `u32::MAX`.
///
/// # Examples
///
/// ```
...
/// .unwrap();
///
/// let pruned = prune_csr(m, 1e-10).unwrap();
/// assert_eq!(pruned.nnz(), 1);
/// assert_eq!(pruned.col_indices(), &[1]);
/// assert_eq!(pruned.values(),      &[2.0]);
/// ```
pub fn prune_csr<T: Scalar + PartialOrd>(
    m: CsrMatrix<T>,
    tolerance: T,
) -> Result<CsrMatrix<T>, CsrError> {
    ...
    let mut new_row_ptr = vec![0u32; expected_len];
    ...
    for r in 0..rows {
        ...
        new_row_ptr[r + 1] =
            u32::try_from(new_col.len()).map_err(|_| CsrError::DimensionOverflow)?;
    }
...
/// # Errors
///
/// Returns [`CscError::DimensionOverflow`] if `cols + 1` overflows `usize`, or if the
/// pruned non-zero count exceeds `u32::MAX`.
///
/// # Examples
///
...
/// .unwrap();
///
/// let pruned = prune_csc(m, 1e-10).unwrap();
/// assert_eq!(pruned.nnz(), 1);
/// assert_eq!(pruned.row_indices(), &[1]);
/// assert_eq!(pruned.values(),      &[2.0]);
/// ```
pub fn prune_csc<T: Scalar + PartialOrd>(
    m: CscMatrix<T>,
    tolerance: T,
) -> Result<CscMatrix<T>, CscError> {
    ...
    let mut new_col_ptr = vec![0u32; expected_len];
    ...
    for c in 0..cols {
        ...
        new_col_ptr[c + 1] =
            u32::try_from(new_row.len()).map_err(|_| CscError::DimensionOverflow)?;
    }
...
mod tests {
    ...
    fn keeps_entries_outside_tolerance_band() {
        ...
        .unwrap();
        let p = prune_csr(m, 1e-10).unwrap();
        assert_eq!(p.nnz(), 1);
    ...
    fn rows_plus_one_overflow_is_an_error_not_a_panic() {
        let m: CsrMatrix<f64> = CsrMatrix::new_raw(usize::MAX, 1, vec![], vec![], vec![]);
        let err = prune_csr(m, 1e-10);
...
mod tests_csc {
    ...
    fn csc_keeps_entries_outside_tolerance_band() {
        ...
        .unwrap();
        let p = prune_csc(m, 1e-10).unwrap();
        assert_eq!(p.nnz(), 1);
    ...
    #[test]
    fn cols_plus_one_overflow_is_an_error_not_a_panic() {
        let m: CscMatrix<f64> = CscMatrix::new_raw(1, usize::MAX, vec![], vec![], vec![]);
        let err = prune_csc(m, 1e-10);
        assert_eq!(err, Err(CscError::DimensionOverflow));
    }
}
