...
pub enum CscError {
    ...
    RowIndexOutOfBounds,
    /// Computing `cols + 1` overflowed `usize`.
    DimensionOverflow,
}
...
impl<T> CscMatrix<T> {
    ...
    /// - [`CscError::DimensionOverflow`] — `cols + 1` overflows `usize`.
    ///
    /// # Examples
    ///
    /// ```
...
mod tests {
    ...
    fn cols_plus_one_overflow_is_an_error_not_a_panic() {
        let err = CscMatrix::<f64>::new(1, usize::MAX, vec![], vec![], vec![]);
