...
pub(crate) fn run() {
    ...
    println!("  row_ptr={:?}  col_indices={:?}  values={:?}", csr.row_ptr(), csr.col_indices(), csr.values());

    // csr_to_coo: expands row_ptr into explicit row indices.
    let csr_eye = CsrMatrix::new(3, 3, vec![0, 1, 2, 3],
                                 vec![0, 1, 2],
                                 vec![1.0_f64, 1.0, 1.0])
        .expect("valid CSR");
    let coo_out = csr_to_coo(csr_eye);
    println!("\ncsr_to_coo (3×3 identity): row_indices={:?}", coo_out.row_indices());
    ...
    println!("\ncsr_to_csc (2×3):");
    println!("  col_ptr={:?}  row_indices={:?}  values={:?}", csc_out.col_ptr(), csc_out.row_indices(), csc_out.values());

    // csc_to_csr: transposes back, sorted by (row, col).
    let csc_eye = CscMatrix::new(3, 3, vec![0, 1, 2, 3], vec![0, 1, 2], vec![1.0_f64, 1.0, 1.0])
        .expect("valid CSC");
    let csr_back = csc_to_csr(csc_eye);
    println!("\ncsc_to_csr (3×3 identity): row_ptr={:?}", csr_back.row_ptr());
}
